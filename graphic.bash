...
coordinate_invalid(){
    ...
    if ((0<=x<=CANVAS_WIDTH && 1<=y<=CANVAS_HEIGHT))
    then
        return 1
    else
        return 0
...
xy_to_index(){
    local x="${1:?}"
    local y="${2:?}"
    # who needs the first 11 indexes in a bash array anyways
    echo $((x*CANVAS_HEIGHT + y))
}

# init_canvas <width> <height> [<color_index>]
init_canvas(){
    # process stuff
    declare -rg CANVAS_WIDTH="${1:?Must provide a width}" CANVAS_HEIGHT="${2:?Must provide a height}" DRAW_CHAR='▀' DEFAULT_COLOR="${3:-0}"

    # Well just use the Buffer to store any color the canvas is updated with
